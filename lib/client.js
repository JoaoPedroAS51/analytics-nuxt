  ...
  if (process.client) {
    ...
    if (app.hasOwnProperty('$analytics') && config.useRouter) {
      app.router.afterEach((to, from) => {
        const titleChunk = app.head.title
        const titleTemplateChunk = app.head.titleTemplate
        let title = titleChunk

        if(titleTemplateChunk) {
          if(typeof titleTemplateChunk === 'string' && titleChunk) {
            title = titleTemplateChunk.replace('%s', titleChunk)
          } else if(typeof titleTemplateChunk === 'function') {
            title = titleTemplateChunk(titleChunk)
          }
        }
